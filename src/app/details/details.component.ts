    ...
    <div class="container">
      <div class="row">
          <div class="col-12 col-md-6 col-lg-6 col-xl-8">
              ...
              <span class="badge rounded-pill text-bg-primary">{{ housingLocation?.type }}</span><p class="card-text">{{housingLocation?.rate}}</p>
              <small class="text-body-secondary">{{housingLocation?.name}}</small>
              <p><i class="fa-solid fa-location-dot"></i> {{housingLocation?.place}} , {{housingLocation?.city}} <a [href]="housingLocation?.map" target="_blank">แผนที่</a></p>
          ...
          <div class="col-12 col-xl-4 closeright">
              ...
              <br>
              <a [routerLink]="['/form', housingLocation?.id]" class="btn btn-primary" style="--bs-btn-padding-y: 5px; --bs-btn-padding-x: 80px; --bs-btn-font-size: 20px;">จองเลย</a>
              <br><br>
          </div>
            ...
            <!--Pic-->
              <div class="carousel-inner slidecurve">
                  <div class="justify-content-center align-items-center">
                      <img [src]="housingLocation?.photo" class="w-100 modalslideimg" style="height: 650px;" alt="...">
                  </div>
              </div>
            <br>
          </div>
...
export class DetailsComponent {
  ...
  constructor() {
    ...
    this.housingService.getHousingLocationById(housingLoactionId).then(housingLoactionId => {
